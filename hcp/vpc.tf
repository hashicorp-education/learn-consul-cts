...
resource "aws_security_group" "secgrp_jumphost" {
  name        = "secgrp_jumphost"
...
resource "aws_security_group_rule" "ingress_jumphost_rule" {
  from_port         = 22
  to_port           = 22
  cidr_blocks       = ["0.0.0.0/0"]
  ipv6_cidr_blocks  = ["::/0"]
...
resource "aws_instance" "jumphost" {
  ...
  vpc_security_group_ids = [
    aws_security_group.secgrp_jumphost.id
  ]
