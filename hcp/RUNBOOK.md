...
```
$ aws --region eu-central-1  ec2 describe-instances --filters "Name=tag-key,Values=CtsJumphostModule" | jq '.Reservations[].Instances[].State.Name'
"running"
```
...
```
$ aws --region eu-central-1 ec2 describe-security-groups --filters "Name=tag-key,Values=CtsJumphostModule" | jq -r '.SecurityGroups[].IpPermissionsEgress'
[
  {
    "FromPort": 22,
    "IpProtocol": "tcp",
...
```
$ aws --region eu-central-1 ec2 terminate-instances --instance-ids $(aws --region eu-central-1 ec2 describe-instances --filters "Name=tag-key,Values=CtsJumphostModule" | jq -r '.Reservations[].Instances[].InstanceId')
{
...
```
$ aws --region eu-central-1 ec2 delete-key-pair --key-pair-id $(aws --region eu-central-1 ec2 describe-key-pairs --filters "Name=tag-key,Values=CtsJumphostModule" | jq -r '.KeyPairs[].KeyPairId')
...
```
$ aws --region eu-central-1 ec2 delete-security-group --group-id $(aws --region eu-central-1 ec2 describe-security-groups --filters "Name=tag-key,Values=CtsJumphostModule" | jq -r '.SecurityGroups[].GroupId')
